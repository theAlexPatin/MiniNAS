...
function getInviteStatus(invite: { used_at: string | null; expires_at: string }): "used" | "expired" | "pending" {
...
function VolumeAccessPanel({ volumeId, allUsers }: { volumeId: string; allUsers: { id: string; username: string }[] }) {
...
function VolumesSection() {
  ...
  const handleAdd = async () => {
    ...
    await addMutation.mutateAsync({ id: newId.trim(), label: newLabel.trim(), path: newPath.trim() });
    setNewId("");
    setNewLabel("");
    setNewPath("");
  };
  ...
  const handleRemove = (vol: AdminVolume) => {
    if (confirm(`Remove volume "${vol.label}" (${vol.id})? This only removes it from MiniNAS — files on disk are not deleted.`)) {
            ...
            <div key={vol.id} className="px-5 py-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  >
                    {expandedVolume === vol.id ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                  </button>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-800">{vol.label}</p>
                    <p className="text-xs text-gray-400 font-mono truncate">{vol.id} &middot; {vol.path}</p>
                  </div>
                </div>
              ...
              {expandedVolume === vol.id && (
                <VolumeAccessPanel
                  volumeId={vol.id}
                  allUsers={(usersData?.users || []).map((u) => ({ id: u.id, username: u.username }))}
                />
              )}
            </div>
          ))}
