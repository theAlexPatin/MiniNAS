...
export default function ShareDialog({ file, volume, onClose }: ShareDialogProps) {
	...
	const handleCreate = async () => {
		...
		try {
			...
			const url =
				data.url ||
				`${window.location.origin}${withBase(`/api/v1/share/${data.share.id}/download`)}`
			setShareUrl(url)
		} catch (err) {
			...
			>
				...
				{shareUrl ? (
					<div>
						...
						{password && <p className="text-xs text-gray-400 mt-2">Password: {password}</p>}
					</div>
				) : (
					<div className="space-y-3">
						<div>
							<label className="block text-sm text-gray-500 mb-1">Password (optional)</label>
							<input
								type="text"
								value={password}
						...
						<div>
							<label className="block text-sm text-gray-500 mb-1">Max downloads (optional)</label>
							<input
								type="number"
								value={maxDownloads}
						...
						<div>
							<label className="block text-sm text-gray-500 mb-1">Expires in (hours)</label>
							<select
								value={expiresIn}
