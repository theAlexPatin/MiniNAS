...
import { api, discoverVolumes, prompt } from '../helpers.js'
...
export function registerVolumeCommands(program: Command, getOpts: () => ApiOpts): void {
	...
	volume
		...
		.action(async () => {
			...
			const visW = Math.max(10, ...rows.map((r: any) => (r.visibility || 'public').length))
			console.log(
	...
	volume
		...
		.action(async (id?: string, label?: string, volumePath?: string) => {
			if (!id) {
				...
				const available = discoverVolumes().filter((v) => !registered.has(v.path))
	...
	const access = volume.command('access').description('Manage volume access')
	...
	access
		...
		.action(async (volumeId: string) => {
			...
			if (rows.length === 0) {
				console.log(`No explicit access grants for volume '${volumeId}'.`)
				return
			}
			const idW = Math.max(2, ...rows.map((r: any) => r.id.length))
	...
	access
		...
		.action(async (volumeId: string, userId: string) => {
			...
			console.log(`Granted access to volume '${volumeId}' for user '${userId}'.`)
		})
	...
	access
		...
		.action(async (volumeId: string, userId: string) => {
			...
			console.log(`Revoked access to volume '${volumeId}' for user '${userId}'.`)
		})
}
