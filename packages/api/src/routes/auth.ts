...
import { markInviteUsed, validateInvite } from '../services/invites.js'
...
auth.post('/registration/verify', async (c) => {
	...
	try {
		...
		db.prepare("INSERT OR IGNORE INTO users (id, username, role) VALUES (?, ?, 'admin')").run(
			pendingUserId,
			'admin',
		)
...
auth.post('/invite/verify', async (c) => {
	...
	try {
		...
		db.prepare("INSERT INTO users (id, username, role) VALUES (?, ?, 'user')").run(
			pendingUserId,
			inviteResult.invite.username,
		)
...
auth.get('/authentication/options', async (c) => {
	...
	const credentials = db.prepare('SELECT id, transports FROM credentials').all() as Pick<
		DbCredential,
		'id' | 'transports'
	>[]
...
auth.get('/session', async (c) => {
	...
	const user = db
		...
		.get(identity.userId) as { id: string; username: string; role: string } | undefined
