...
export function createInvite(
	...
	return db.prepare('SELECT * FROM invite_tokens WHERE id = ?').get(id) as InviteToken
}

export function validateInvite(id: string): {
	valid: boolean
	error?: string
	invite?: InviteToken
} {
	const db = getDb()
	const invite = db.prepare('SELECT * FROM invite_tokens WHERE id = ?').get(id) as
		| InviteToken
		| undefined
...
export function markInviteUsed(id: string, userId: string): void {
	const db = getDb()
	db.prepare("UPDATE invite_tokens SET used_by = ?, used_at = datetime('now') WHERE id = ?").run(
		userId,
		id,
	)
}
...
export function listInvites(): InviteToken[] {
	const db = getDb()
	return db.prepare('SELECT * FROM invite_tokens ORDER BY created_at DESC').all() as InviteToken[]
}
