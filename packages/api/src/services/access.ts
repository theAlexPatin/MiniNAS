...
export function canAccessVolume(userId: string, volumeId: string): boolean {
  ...
  const db = getDb();
  const volume = db
    .prepare("SELECT visibility FROM volumes WHERE id = ?")
    .get(volumeId) as { visibility: string } | undefined;
  ...
  const access = db
    .prepare(
      "SELECT 1 FROM volume_access WHERE volume_id = ? AND user_id = ?"
    )
    .get(volumeId, userId);
  return !!access;
}
...
export function getAccessibleVolumeIds(userId: string): string[] {
  ...
  if (isAdmin(userId)) {
    const rows = db
      .prepare("SELECT id FROM volumes")
      .all() as { id: string }[];
...
export function grantVolumeAccess(volumeId: string, userId: string): void {
  const db = getDb();
  db.prepare(
    "INSERT OR IGNORE INTO volume_access (volume_id, user_id) VALUES (?, ?)"
  ).run(volumeId, userId);
}
...
export function revokeVolumeAccess(volumeId: string, userId: string): void {
  const db = getDb();
  db.prepare(
    "DELETE FROM volume_access WHERE volume_id = ? AND user_id = ?"
  ).run(volumeId, userId);
}

export function getVolumeAccessList(
  volumeId: string
): UserInfo[] {
  const db = getDb();
  return db
...
export function setVolumeVisibility(
  volumeId: string,
  visibility: "public" | "private"
): void {
  const db = getDb();
  db.prepare("UPDATE volumes SET visibility = ? WHERE id = ?").run(
    visibility,
    volumeId
  );
}
