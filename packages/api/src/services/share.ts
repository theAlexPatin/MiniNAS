...
export function createShareLink(opts: {
  ...
  db.prepare(
    `INSERT INTO share_links (id, user_id, volume, path, password_hash, max_downloads, expires_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(id, opts.userId, opts.volume, opts.path, passwordHash, opts.maxDownloads || null, expiresAt);
