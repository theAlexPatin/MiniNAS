...
describe('GET /api/v1/share/:id/download', () => {
	...
	it('rejects wrong password', async () => {
		...
		const res = await app.request(`${API}/${link.id}/download?password=wrong`)
		expect(res.status).toBe(403)
	})
	...
	it('downloads with correct password', async () => {
		...
		const res = await app.request(`${API}/${link.id}/download?password=secret`)
		expect(res.status).toBe(200)
		const text = await res.text()
		expect(text).toBe(testFileContent)
	})
...
describe('publicShareApp - GET /s/:id', () => {
	...
	it('enforces password on public shares', async () => {
		...
		// Wrong password
		const res2 = await publicShareApp.request(`/s/${link.id}?password=wrong`)
		expect(res2.status).toBe(403)

		// Correct password
		const res3 = await publicShareApp.request(`/s/${link.id}?password=secret`)
