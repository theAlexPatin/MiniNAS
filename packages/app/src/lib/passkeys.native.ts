...
export async function registerPasskey(): Promise<boolean> {
  ...
  throw new Error('Passkey registration is not yet implemented on native. Use the web app to register.')
}

export async function authenticatePasskey(): Promise<boolean> {
  // TODO: Implement native passkey authentication
  throw new Error('Passkey authentication is not yet implemented on native. Use the web app to sign in.')
}
