...
import { AlertCircle, CheckCircle, ChevronDown, ChevronUp, Pause, Play, X } from 'lucide-react-native'
        ...
        <View style={styles.list}>
          {uploads.map((item) => (
            <View key={item.id} style={styles.item}>
              {item.status === 'complete' && <CheckCircle size={16} color={Colors.StatusColor.success} />}
              {item.status === 'error' && <AlertCircle size={16} color={Colors.StatusColor.error} />}

              <View style={styles.itemInfo}>
                <Text style={styles.itemName} numberOfLines={1}>{item.file.name}</Text>
