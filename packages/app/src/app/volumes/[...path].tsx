  ...
  const navigateTo = useCallback(
    (newPath: string) => {
      ...
      const segments = newPath
        ? `${encodeURIComponent(volume)}/${newPath
            .split('/')
      ...
      if (Platform.OS === 'web') {
        ...
        if (newName && newName !== file.name) {
          ...
          api.moveFile(volume, file.path, destination).then(
            ...
            (err) =>
              addToast('error', `Rename failed: ${(err as Error).message}`),
          )
        }
      }
    },
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <ScrollView
        ...
        contentContainerStyle={[
          styles.content,
          selectionCount > 0 && { paddingBottom: 80 },
        ]}
      >
        {/* Header */}
            ...
            {volume ? (
              <Pressable
                ...
                style={[
                  ...
                  showUploadZone && styles.toolbarButtonActive,
                ]}
              >
                <Upload
                  size={16}
                  color={
                    showUploadZone ? '#fff' : Colors.TextColor.secondary
                  }
                />
              </Pressable>
            ) : null}
        ...
        {!volume ? (
          ...
          <FileList
            ...
            onSelectAll={(paths) =>
              paths.length > 0 ? selectAll(paths) : clearSelection()
            }
          />
        ) : (
          <FileGrid
