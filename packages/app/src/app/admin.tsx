        ...
        data.volumes.map((vol) => (
          <View key={vol.id} style={styles.volumeRow}>
            <View style={styles.volumeInfo}>
              <Pressable onPress={() => setExpandedVolume(expandedVolume === vol.id ? null : vol.id)}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.volPath}>{vol.id} · {vol.path}</Text>
              </View>
              <Pressable onPress={() => toggleVisibility(vol)} style={[
                styles.visBadge,
                vol.visibility === 'public' ? styles.visBadgePublic : styles.visBadgePrivate,
              ]}>
                {vol.visibility === 'public' ? <Eye size={12} color="#047857" /> : <EyeOff size={12} color={Colors.TextColor.secondary} />}
                <Text style={vol.visibility === 'public' ? styles.visTextPublic : styles.visTextPrivate}>
                  {vol.visibility}
                </Text>
              </Pressable>
            ...
            {expandedVolume === vol.id && vol.visibility === 'private' && (
              <VolumeAccessPanel
                volumeId={vol.id}
                allUsers={(usersData?.users || []).map((u) => ({ id: u.id, username: u.username, role: u.role }))}
              />
            )}
    ...
    <View style={styles.card}>
      ...
      <View style={styles.createForm}>
        ...
        <Pressable
          ...
          style={[styles.createBtn, (!username.trim() || createMutation.isPending) && { opacity: 0.5 }]}
        >
...
const styles = StyleSheet.create({
  ...
  screenContent: { maxWidth: 720, alignSelf: 'center', width: '100%', padding: 16, paddingBottom: 40 },
  ...
  card: { backgroundColor: '#ffffff', borderWidth: 1, borderColor: Colors.BorderColor.primary, borderRadius: Outlines.borderRadius.lg, ...Outlines.shadow.sm },
  cardHeader: { paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: Colors.BorderColor.secondary },
  cardTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  cardTitle: { fontSize: 14, fontWeight: '600', color: Colors.TextColor.primary },
  volumeRow: { paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.BorderColor.secondary },
  volumeInfo: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  volLabel: { fontSize: 14, fontWeight: '500', color: Colors.TextColor.primary },
  volPath: { fontSize: 11, color: Colors.TextColor.tertiary, fontFamily: Typography.caption.fontFamily },
  visBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: Outlines.borderRadius.md, borderWidth: 1 },
  visBadgePublic: { borderColor: '#a7f3d0', backgroundColor: '#ecfdf5' },
  visBadgePrivate: { borderColor: Colors.BorderColor.primary, backgroundColor: Colors.BackgroundColor.secondary },
  visTextPublic: { fontSize: 11, color: '#047857' },
  visTextPrivate: { fontSize: 11, color: Colors.TextColor.secondary },
  accessPanel: { marginTop: 12, paddingLeft: 16, borderLeftWidth: 2, borderLeftColor: Colors.BorderColor.secondary },
  accessTitle: { fontSize: 11, fontWeight: '500', color: Colors.TextColor.secondary, marginBottom: 8 },
  ...
  userRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: Colors.BorderColor.secondary },
  ...
  resetBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: Outlines.borderRadius.md, borderWidth: 1, borderColor: Colors.BorderColor.primary, marginRight: 8 },
  resetBtnText: { fontSize: 11, color: Colors.TextColor.secondary },
  deleteBtn: { padding: 6, borderRadius: Outlines.borderRadius.md },
  createForm: { flexDirection: 'row', gap: 8, paddingHorizontal: 20, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: Colors.BorderColor.secondary },
  formInput: { flex: 1, backgroundColor: Colors.BackgroundColor.secondary, borderWidth: 1, borderColor: Colors.BorderColor.primary, borderRadius: Outlines.borderRadius.md, paddingHorizontal: 12, paddingVertical: 6, fontSize: 13, color: Colors.TextColor.primary },
  createBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: Colors.BrandColor[600], paddingHorizontal: 12, paddingVertical: 6, borderRadius: Outlines.borderRadius.md },
